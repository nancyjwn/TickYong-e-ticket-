...
@section('content')
    <div class="container mx-auto p-8">
        ...
        <form method="GET" action="{{ route('dashboard.events.search') }}" class="mb-8">
            <div class="flex items-center space-x-6">
                ...
                <select name="category" id="categoryDropdown"
                    class="p-4 text-xl text-gray-900 border border-gray-300 rounded-xl focus:ring-4 focus:ring-red-500 transition shadow-lg">
        ...
        @else
            <div id="eventsContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
    ...
    <script>
        $(document).ready(function() {
            // Mengambil data ketika kategori berubah
            $('#categoryDropdown').change(function() {
                var categoryId = $(this).val(); // ID kategori yang dipilih
                var query = $("input[name='query']").val(); // Mengambil query pencarian

                // Melakukan request AJAX ke route pencarian
                $.ajax({
                    ...
                    success: function(response) {
                        // Menampilkan hasil event yang diterima dari server
